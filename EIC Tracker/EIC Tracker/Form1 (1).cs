...
- Change font from Euro Caps to Sintony (along with some sizing/positioning changes).
- Recording "bounty" event - occurs when you destroy a ship.
...
- Earyn would like to filter the shown systems by tick date. Allow the ability to clear the board (hide completed missions, hide turn-ins [trade, pawned, combat, bonds, exploration]).
...
namespace EIC_Tracker
{
    public partial class frmMain : Form
    {
        ...
        public static class Globals
        {
            ...
            public static string curshipname = "";
            public static string curshipident = "";
            ...
            public static string version = "1.0.91"; //Version Number
        ...
        {
            lblShip.Text = "Flying: " + Globals.curshipname + " [" + Globals.curshipident + "] (" + Globals.curship + ")";
                ...
                case "Location":
                    ...
                    if (line.timestamp > Globals.curtime) //If this is a current record (FSDJump processes all the time)
                    {
                        ...
                        if (line.Factions != null)
                        {
                            ...
                            {
                                //We've got a list of factions!
                                foreach (var faction in line.Factions)
                                {
                                    //Create variable to determine if the faction is the control faction by comparing the name of the faction to the SystemFaction value.
                                    int val = 0;
                                    if(line.SystemFaction != null) //SystemFaction value is present.
                                    {
                                        if(line.SystemFaction == (string)faction.Name)
                                        {
                                            val = 1;
                                        }
                                    }else
                                    {
                                        val = 2; //A value of 2 will tell the tracker NOT to update the control value, just incase the SystemFaction wasn't present. (Saves it overriding correct records).
                                    }
                                    string inf = System.Convert.ToString(faction.Influence);
                                    TrackData(line.StarSystem, "FACTION", val, (string)faction.Name, (string)inf, (string)faction.FactionState);
                                }
                            }
                        }
                    }
                        ...
                        switch ((string)line.eventtype)
                        {
                            ...
                            case "MissionAccepted":
                                ...
                                //if (line.Commodity_Localised != null)
                                {
                                    ...
                                    */
                                    if (line.Name.ToUpper().Contains("COLLECT"))
                                    {
                                        line.Type = "COLLECT";
                                    ...
                                    else if (line.Name.ToUpper().Contains("CREDITS"))
                                    {
                                        line.Type = "DONATION";
                            ...
                            case "RedeemVoucher":
                                switch ((string)line.Type.ToUpper())
                                {
                                    ...
                                    case "BOUNTY":
                                        ...
                                        {
                                            ClaimFaction = line.Faction;
                                        }

                                        //Track the redeeming of BH.
                                        //BH vouchers are for the faction who issued the voucher, this isn't in the journal atm.
                                        TrackData(Globals.cursystem.ToUpper(), "BH", Convert.ToInt64(line.Amount), ClaimFaction);
        ...
        public void FindCommander()
        {
            ...
            {
                //Add a completed handler so that we know when to submit the form.
                var url = "http://eicgaming.com/tracker/commander.php?CMDR=" + Globals.cmdr + "&x=" + Globals.cursystemx + "&y=" + Globals.cursystemy + "&z=" + Globals.cursystemz + "&System=" + Globals.cursystem;
        ...
        public void FindCommodity()
        {
            if (cmbCommodity.SelectedItem != null)
            {
                ...
                var url = "http://eicgaming.com/tracker/commodity.php?CMDR=" + Globals.cmdr + "&Commodity=" + commtext + "&x=" + Globals.cursystemx + "&y=" + Globals.cursystemy + "&z=" + Globals.cursystemz + "&System=" + Globals.cursystem;
                if (Globals.curstation != "")
                {
